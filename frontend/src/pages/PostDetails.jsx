...
const fetchPost= async()=>{
  setLoader(true)
  try {
    const result = await axios.get(URL+"/api/posts/"+postId)
      ...
      {loader?<div className="h-[80vh] flex justify-center items-center w-full"><Loader/></div>:<div className="px-8 md:px-[200px] mt-8">
        <div className="flex justify-between items-center">
            ...
            {user?._id===post?.userId && <div className="flex items-center justify-center space-x-2">
                <p className="cursor-pointer" onClick={()=>navigate("/edit/"+postId)} ><CiEdit /> </p>
                <p className="cursor-pointer" onClick={handleDeletePost}><MdDeleteOutline /> </p>
            </div>}
